...
def get_traj_from_ruckig(q0, q0_dot, q0_dotdot, qd, qd_dot, qd_dotdot, margin_velocity=1.0, margin_acceleration=0.7, margin_jerk=MARGIN_JERK):
...
trajectory = get_traj_from_ruckig(qs, qs_dot, qs_dotdot, qd, qd_dot, qd_dotdot, margin_velocity=MARGIN_VELOCITY, margin_acceleration=MARGIN_ACCELERATION)
trajectory_back = get_traj_from_ruckig(qd, qd_dot, qd_dotdot, qs, qs_dot, qs_dotdot, margin_velocity=MARGIN_VELOCITY, margin_acceleration=MARGIN_ACCELERATION*0.5)
...
class Throwing_controller:    
    def __init__(self):
        ...
        self.target_state_pub = rospy.Publisher('/robot/arm/computed_torque_controller/target_state', JointState, queue_size=1)
    ...
    def simulate_trajectory(self, trajectory):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=2.5, cameraYaw=145, cameraPitch=-45, cameraTargetPosition=[0.8,0,0])
        ...
        robotId = p.loadURDF("descriptions/URDF_Assembly_Arm_v0_1_SLDASM/urdf/TUMArm_v0_1_SLDASM_Robotiq.urdf", [0.0,0.0,0.0], useFixedBase=True, flags=p.URDF_USE_INERTIA_FROM_FILE)
        ...
        p.resetJointStatesMultiDof(robotId, controlled_joints, [[q0_i] for q0_i in q0], targetVelocities=[[q0_i] for q0_i in np.zeros(3)])
    ...
    def run_simulation(self):
        ...
        trajectory_to_qs = get_traj_from_ruckig(q0, np.zeros(3), np.zeros(3), qs, qs_dot, qs_dotdot, margin_velocity=0.2, margin_acceleration=0.1)
        ...
        while not rospy.is_shutdown():
            ...
            if self.fsm_state == "THROWING" or self.fsm_state == "RELEASE":
                if (self.time_throw - rospy.get_time()) < 10e-3:
                    self.tracking_error_pos.append(np.array(self.target_state.position) - np.array(self.robot_state.position)[-3:])
                    self.tracking_error_vel.append(np.array(self.target_state.velocity) - np.array(self.robot_state.velocity)[-3:])
            ...
            if self.fsm_state == "IDLE":
                ...
                self.homing_traj = get_traj_from_ruckig(self.q0, np.zeros(3), np.zeros(3), qs, qs_dot, qs_dotdot, margin_velocity=0.2, margin_acceleration=0.1)
                ...
                if time_now - self.time_start_throwing > self.throwing_traj.duration-dT:
                    ...
                    self.trajectory_back = get_traj_from_ruckig(q_cur, qdot_cur, qd_dotdot, qs, qs_dot, qs_dotdot, margin_velocity=MARGIN_VELOCITY, margin_acceleration=MARGIN_ACCELERATION*0.5)
                    print("Slowing...")
    ...
    def convert_command_to_ROS(self, time_now, qd, qd_dot, qd_dotdot):
        ...
        command.name = ['panda_joint1','panda_joint2','panda_joint3','panda_joint4','panda_joint5','panda_joint6','panda_joint7']
    ...
    def scheduler_callback(self, msg):
        ...
        if self.fsm_state == "IDLE_THROWING" and msg.data==1:
            ...
            q_cur = np.array(self.robot_state.position)[-3:]
            qdot_cur = np.array(self.robot_state.velocity)[-3:]
            self.throwing_traj = get_traj_from_ruckig(q_cur, qdot_cur, np.zeros(3), qd, qd_dot, qd_dotdot, margin_velocity=MARGIN_VELOCITY, margin_acceleration=MARGIN_ACCELERATION)
